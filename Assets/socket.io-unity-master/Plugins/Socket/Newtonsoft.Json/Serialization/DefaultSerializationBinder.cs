		...
		private Type GetTypeFromTypeNameKey(TypeNameKey typeNameKey)
		{
			...
			if (assembly1 == null)
				throw new JsonSerializationException(
				  "Could not load assembly '{0}'.".FormatWith(CultureInfo.InvariantCulture,
					 assemblyName));
			...
			if (type == null)
			{
				if (typeName.IndexOf('`') >= 0)
				{
					...
					{
						throw new JsonSerializationException(
						  "Could not find type '{0}' in assembly '{1}'.".FormatWith(CultureInfo.InvariantCulture,
							 typeName, assembly1.FullName), ex);
					}
				}

				if (type == null)
					throw new JsonSerializationException("Could not find type '{0}' in assembly '{1}'.".FormatWith(
					   CultureInfo.InvariantCulture, typeName, assembly1.FullName));
			}
