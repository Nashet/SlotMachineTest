	...
	internal static class JsonTypeReflector
	{
		  ...
		  bool ignoreSerializableAttribute)
		{
			JsonObjectAttribute cachedAttribute =
			  JsonTypeReflector.GetCachedAttribute<JsonObjectAttribute>(objectType);
			if (cachedAttribute != null)
				return cachedAttribute.MemberSerialization;
			return !ignoreSerializableAttribute && JsonTypeReflector.IsSerializable(objectType)
		...
		private static Func<object[], object> GetCreator(Type type)
		{
			...
			return parameters =>
			{
				try
				{
					if (parameters != null)
					{
						ConstructorInfo constructor = type.GetConstructor(parameters
						  .Select<object, Type>(param => param.GetType()).ToArray<Type>());
						if (constructor != null)
				...
				{
					throw new JsonException(
					  "Error creating '{0}'.".FormatWith(CultureInfo.InvariantCulture, type), ex);
				}
			};
		}
		...
		{
			T attribute1 = ReflectionUtils.GetAttribute<T>(type, true);
			if (attribute1 != null)
				return attribute1;
		...
		{
			T attribute1 = ReflectionUtils.GetAttribute<T>(memberInfo, true);
			if (attribute1 != null)
				return attribute1;
			if (memberInfo.DeclaringType != null)
			{
				foreach (Type targetType in memberInfo.DeclaringType.GetInterfaces())
				{
					...
					{
						T attribute2 = ReflectionUtils.GetAttribute<T>(memberInfoFromType, true);
						if (attribute2 != null)
							return attribute2;
					}
				}
