...
namespace Socket.Newtonsoft.Json.Serialization
{
	internal class JsonSerializerInternalWriter : JsonSerializerInternalBase
	{
		...
		public void Serialize(JsonWriter jsonWriter, object value, Type objectType)
		{
			...
			try
			{
				if (this.ShouldWriteReference(value, null, contractSafe, null,
				   null))
					this.WriteReference(jsonWriter, value);
				else
					this.SerializeValue(jsonWriter, value, contractSafe, null, null,
					   null);
			}
			catch (Exception ex)
			{
				if (this.IsErrorHandled(null, contractSafe, null, null, jsonWriter.Path,
				  ex))
				{
		  ...
		  JsonProperty containerProperty)
		{
			if (contract.TypeCode == PrimitiveTypeCode.Bytes && this.ShouldWriteType(TypeNameHandling.Objects,
				   contract, member, containerContract, containerProperty))
			{
					...
					switch (valueContract.ContractType)
					{
						...
						case JsonContractType.Dictionary:
							...
							this.SerializeDictionary(writer,
							  value is IDictionary ? (IDictionary)value : contract2.CreateWrapper(value), contract2,
		  ...
		  JsonProperty containerProperty)
		{
				  ...
				  : (this._serializeStack.Contains(value) ? 1 : 0)) != 0)
			{
				...
				if (property != null)
					str += " for property '{0}'".FormatWith(CultureInfo.InvariantCulture,
					   property.PropertyName);
				string message = str + " with type '{0}'.".FormatWith(CultureInfo.InvariantCulture,
									value.GetType());
				switch (nullable.GetValueOrDefault(this.Serializer._referenceLoopHandling))
				{
					case ReferenceLoopHandling.Error:
						throw JsonSerializationException.Create(null, writer.ContainerPath, message,
						   null);
		...
		private string GetReference(JsonWriter writer, object value)
		{
			try
			{
				return this.Serializer.GetReferenceResolver().GetReference(this, value);
			}
			catch (Exception ex)
			{
		...
		private void OnSerializing(JsonWriter writer, JsonContract contract, object value)
		{
			if (this.TraceWriter != null && this.TraceWriter.LevelFilter >= TraceLevel.Info)
				this.TraceWriter.Trace_(TraceLevel.Info,
				  JsonPosition.FormatMessage(null, writer.Path,
					"Started serializing {0}".FormatWith(CultureInfo.InvariantCulture,
		...
		private void OnSerialized(JsonWriter writer, JsonContract contract, object value)
		{
			if (this.TraceWriter != null && this.TraceWriter.LevelFilter >= TraceLevel.Info)
				this.TraceWriter.Trace_(TraceLevel.Info,
				  JsonPosition.FormatMessage(null, writer.Path,
					"Finished serializing {0}".FormatWith(CultureInfo.InvariantCulture,
					   contract.UnderlyingType)), null);
		  ...
		  JsonProperty containerProperty)
		{
			...
			for (int index = 0; index < contract.Properties.Count; ++index)
			{
				...
				try
				{
					...
					object memberValue;
					if (this.CalculatePropertyValues(writer, value, contract, member, property,
					  out memberContract, out memberValue))
					{
						property.WritePropertyName(writer);
						this.SerializeValue(writer, memberValue, memberContract, property, contract,
						  member);
					}
				}
				catch (Exception ex)
				{
					if (this.IsErrorHandled(value, contract, property.PropertyName, null,
			...
			if (keyValuePairs != null)
			{
				foreach (KeyValuePair<object, object> keyValuePair in keyValuePairs)
				{
					...
					string name = contract.ExtensionDataNameResolver != null
					  ...
					  : propertyName;
					if (this.ShouldWriteReference(keyValuePair.Value, null, contractSafe2,
					   contract, member))
					{
						writer.WritePropertyName(name);
						this.WriteReference(writer, keyValuePair.Value);
					}
					else if (this.CheckForCircularReference(writer, keyValuePair.Value, null, contractSafe2,
					   contract, member))
					{
						writer.WritePropertyName(name);
						this.SerializeValue(writer, keyValuePair.Value, contractSafe2, null,
						   contract, member);
					}
				}
			}
				...
				(this.ShouldSerialize(writer, property, value) && this.IsSpecified(writer, property, value)))
			{
				...
				if (this.ShouldWriteProperty(memberValue, property))
				{
					...
					if (memberValue == null)
					{
						...
						{
							Required? itemRequired = jsonObjectContract?.ItemRequired;
		...
		private void WriteReferenceIdProperty(JsonWriter writer, Type type, object value)
		{
			...
			if (this.TraceWriter != null && this.TraceWriter.LevelFilter >= TraceLevel.Verbose)
				this.TraceWriter.Trace_(TraceLevel.Verbose,
				  JsonPosition.FormatMessage(null, writer.Path,
					"Writing object reference Id '{0}' for {1}.".FormatWith(CultureInfo.InvariantCulture,
					   reference, type)), null);
		...
		private void WriteTypeProperty(JsonWriter writer, Type type)
		{
			...
			if (this.TraceWriter != null && this.TraceWriter.LevelFilter >= TraceLevel.Verbose)
				this.TraceWriter.Trace_(TraceLevel.Verbose,
				  JsonPosition.FormatMessage(null, writer.Path,
					"Writing type name '{0}' for {1}.".FormatWith(CultureInfo.InvariantCulture,
					   typeName, type)), null);
		  ...
		  JsonProperty containerProperty)
		{
			...
			{
				if (!this.CheckForCircularReference(writer, value, null, contract, collectionContract,
				  containerProperty))
					return;
				this._serializeStack.Add(value);
				if (this.TraceWriter != null && this.TraceWriter.LevelFilter >= TraceLevel.Info)
		  ...
		  JsonProperty containerProperty)
		{
			...
			object obj1 = wrappedCollection != null ? wrappedCollection.UnderlyingCollection : values;
			this.OnSerializing(writer, contract, obj1);
			...
			foreach (object obj2 in values)
			{
				try
				{
					...
					if (this.ShouldWriteReference(obj2, null, jsonContract, contract,
					  member))
						this.WriteReference(writer, obj2);
					else if (this.CheckForCircularReference(writer, obj2, null, jsonContract,
					   contract, member))
						this.SerializeValue(writer, obj2, jsonContract, null, contract,
						  member);
				}
				catch (Exception ex)
				{
					if (this.IsErrorHandled(obj1, contract, num, null,
					  writer.ContainerPath, ex))
						this.HandleError(writer, top);
					else
						throw;
			...
			if (flag)
				writer.WriteEndObject();
			this._serializeStack.RemoveAt(this._serializeStack.Count - 1);
			this.OnSerialized(writer, contract, obj1);
		}
		  ...
		  JsonProperty containerProperty)
		{
			...
			int num = this.WriteStartArray(writer, values, contract, member, collectionContract, containerProperty)
			  ? 1
			  : 0;
			...
			for (int lowerBound = values.GetLowerBound(length); lowerBound <= values.GetUpperBound(length); ++lowerBound)
			{
				...
				if (indices1.Length == values.Rank)
				{
					...
					try
					{
						...
						if (this.ShouldWriteReference(obj, null, jsonContract, contract,
						  member))
							this.WriteReference(writer, obj);
						else if (this.CheckForCircularReference(writer, obj, null, jsonContract,
						   contract, member))
							this.SerializeValue(writer, obj, jsonContract, null, contract,
							  member);
					}
					catch (Exception ex)
					{
						if (this.IsErrorHandled(values, contract, lowerBound, null,
						  writer.ContainerPath, ex))
		  ...
		  JsonProperty containerProperty)
		{
			if (!JsonTypeReflector.FullyTrusted)
			{
				string message =
				  ("Type '{0}' implements ISerializable but cannot be serialized using the ISerializable interface because the current application is not fully trusted and ISerializable can expose secure data." +
				   ...
				   Environment.NewLine).FormatWith(CultureInfo.InvariantCulture, value.GetType());
				throw JsonSerializationException.Create(null, writer.ContainerPath, message, null);
			}

			this.OnSerializing(writer, contract, value);
			this._serializeStack.Add(value);
			this.WriteObjectStart(writer, value, contract, member, collectionContract,
			  containerProperty);
			...
			foreach (SerializationEntry serializationEntry in info)
			{
				...
				if (this.ShouldWriteReference(serializationEntry.Value, null, contractSafe,
				   contract, member))
				{
					writer.WritePropertyName(serializationEntry.Name);
					this.WriteReference(writer, serializationEntry.Value);
				}
				else if (this.CheckForCircularReference(writer, serializationEntry.Value, null, contractSafe,
				   contract, member))
				{
					writer.WritePropertyName(serializationEntry.Name);
					this.SerializeValue(writer, serializationEntry.Value, contractSafe, null,
					   contract, member);
				}
			}
		  ...
		  JsonProperty containerProperty)
		{
			...
			object currentObject = wrappedDictionary != null ? wrappedDictionary.UnderlyingDictionary : values;
			this.OnSerializing(writer, contract, currentObject);
			this._serializeStack.Add(currentObject);
			this.WriteObjectStart(writer, currentObject, contract, member, collectionContract,
			  containerProperty);
			if (contract.ItemContract == null)
				contract.ItemContract =
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					{
						if (this.IsErrorHandled(currentObject, contract, name, null,
						  writer.ContainerPath, ex))
							this.HandleError(writer, top);
						else
							throw;
			...
			writer.WriteEndObject();
			this._serializeStack.RemoveAt(this._serializeStack.Count - 1);
			this.OnSerialized(writer, contract, currentObject);
		}
			...
			if (contract.ContractType == JsonContractType.Primitive)
			{
				switch (((JsonPrimitiveContract)contract).TypeCode)
				{
					...
					case PrimitiveTypeCode.SingleNullable:
						...
						escape = false;
						return num1.ToString("R", CultureInfo.InvariantCulture);
					...
					case PrimitiveTypeCode.DateTimeNullable:
						...
						DateTimeUtils.WriteDateTimeString(stringWriter, dateTime, writer.DateFormatHandling,
