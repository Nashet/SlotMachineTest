	   ...
	   new RegexConverter()
			...
			if (contractCache1 == null || !contractCache1.TryGetValue(type, out contract))
			{
				...
				lock (this._typeContractCacheLock)
				{
					...
					Dictionary<Type, JsonContract> dictionary = contractCache2 != null
					  ? new Dictionary<Type, JsonContract>(contractCache2)
		...
		protected virtual List<MemberInfo> GetSerializableMembers(Type objectType)
		{
			...
			IEnumerable<MemberInfo> memberInfos = ReflectionUtils
			  ...
			  .Where<MemberInfo>(m => !ReflectionUtils.IsIndexedProperty(m));
		...
		protected virtual JsonObjectContract CreateObjectContract(Type objectType)
		{
			...
			contract.Properties.AddRange<JsonProperty>(
			   this.CreateProperties(contract.NonNullableUnderlyingType,
				contract.MemberSerialization));
			...
			if (cachedAttribute != null)
			{
				...
				if (cachedAttribute.NamingStrategyType != null)
				{
					NamingStrategy namingStrategy =
					  JsonTypeReflector.GetContainerNamingStrategy(cachedAttribute);
					func = s => namingStrategy.GetDictionaryKey(s);
				}
			}
			...
			if (contract.IsInstantiable)
			{
				...
				if (attributeConstructor != null)
				{
					...
					contract.CreatorParameters.AddRange<JsonProperty>(
					   this.CreateConstructorParameters(attributeConstructor, contract.Properties));
				}
				...
				else if (contract.DefaultCreator == null || contract.DefaultCreatorNonPublic)
				{
					...
					if (parameterizedConstructor != null)
					{
						...
						contract.CreatorParameters.AddRange<JsonProperty>(
						   this.CreateConstructorParameters(parameterizedConstructor,
							contract.Properties));
					}
				}
				else if (contract.NonNullableUnderlyingType.IsValueType())
				{
					...
					if (immutableConstructor != null)
					{
						...
						contract.CreatorParameters.AddRange<JsonProperty>(
						   this.CreateConstructorParameters(immutableConstructor, contract.Properties));
					}
				}
			}
		...
		private MemberInfo GetExtensionDataMemberForType(Type type)
		{
			return this.GetClassHierarchyForType(type).SelectMany<Type, MemberInfo>(
			   baseType =>
			   {
				   ...
				   initial.AddRange<MemberInfo>(baseType.GetProperties(
					 BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic));
				   initial.AddRange<MemberInfo>(baseType.GetFields(
					 BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic));
				   return initial;
			   }).LastOrDefault<MemberInfo>(m =>
			  {
				  switch (m.MemberType())
				  {
						  ...
						  if (!ReflectionUtils.CanReadMemberValue(m, true))
							  throw new JsonException(
								"Invalid extension data attribute on '{0}'. Member '{1}' must have a getter.".FormatWith(
								   ...
								   DefaultContractResolver.GetClrTypeFullName(m.DeclaringType), m.Name));
		...
		private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
		{
			...
			if (attribute.ReadData)
			{
				...
				MethodCall<object, object> setExtensionDataDictionaryValue =
				  JsonTypeReflector.ReflectionDelegateFactory.CreateMethodCall<object>(
					 memberUnderlyingType.GetMethod("Add", new Type[2] {
			  genericArgument1,
			  genericArgument2
					}));
			...
			if (attribute.WriteData)
			{
				ObjectConstructor<object> createEnumerableWrapper =
				  JsonTypeReflector.ReflectionDelegateFactory.CreateParameterizedConstructor(
					  typeof(DefaultContractResolver.EnumerableDictionaryWrapper<,>)
		...
		private ConstructorInfo GetAttributeConstructor(Type objectType)
		{
			IEnumerator<ConstructorInfo> enumerator =
			  ...
			  .Where<ConstructorInfo>(
				 c => c.IsDefined(typeof(JsonConstructorAttribute), true)).GetEnumerator();
		  ...
		  ParameterInfo parameterInfo)
		{
			...
			this.SetPropertySettingsFromAttributes(property, parameterInfo, parameterInfo.Name,
		...
		private void InitializeContract(JsonContract contract)
		{
			...
			contract.InternalConverter = JsonSerializer.GetMatchingConverter(
			   BuiltInConverters, contract.NonNullableUnderlyingType);
		...
		private void ResolveCallbackMethods(JsonContract contract, Type t)
		{
			...
			if (onError == null)
				return;
			contract.OnErrorCallbacks.AddRange<SerializationErrorCallback>(onError);
		}
		  ...
		  out List<SerializationErrorCallback> onError)
		{
			...
			{
				MethodInfo currentCallback1 = null;
		  ...
		  Type objectType)
		{
			...
			this.InitializeContract(dictionaryContract);
			...
			if (attribute?.NamingStrategyType != null)
			{
				...
				dictionaryContract.DictionaryKeyResolver = s => namingStrategy.GetDictionaryKey(s);
			}
			else
					...
					if (parameters.Length != 1 || !type2.IsAssignableFrom(parameters[0].ParameterType))
						throw new JsonException(
						  "Constructor for '{0}' must have no parameters or a single parameter that implements '{1}'.".FormatWith(
							 CultureInfo.InvariantCulture, dictionaryContract.UnderlyingType,
							 type2));
					dictionaryContract.HasParameterizedCreator = true;
				}

				dictionaryContract.OverrideCreator =
				  JsonTypeReflector.ReflectionDelegateFactory.CreateParameterizedConstructor(attributeConstructor);
			}
			...
			if (attributeConstructor != null)
			{
				...
				else
				{
					if (parameters.Length != 1 || !type2.IsAssignableFrom(parameters[0].ParameterType))
						throw new JsonException(
						  "Constructor for '{0}' must have no parameters or a single parameter that implements '{1}'.".FormatWith(
							 CultureInfo.InvariantCulture, jsonArrayContract.UnderlyingType,
							 type2));
		  ...
		  Type objectType)
		{
			...
			this.InitializeContract(primitiveContract);
		  ...
		  Type objectType)
		{
			...
			this.InitializeContract(iserializableContract);
			ConstructorInfo constructor = iserializableContract.NonNullableUnderlyingType.GetConstructor(
			  BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, new Type[2] {
		  ...
		  ref Type prevAttributeType)
		{
			...
			if (currentCallback != null)
				throw new JsonException("Invalid attribute. Both '{0}' and '{1}' in type '{2}' have '{3}'.".FormatWith(
				   CultureInfo.InvariantCulture, method, currentCallback,
				   DefaultContractResolver.GetClrTypeFullName(method.DeclaringType), attributeType));
			if (prevAttributeType != null)
				throw new JsonException("Invalid Callback. Method '{3}' in type '{2}' has both '{0}' and '{1}'.".FormatWith(
				   ...
				   DefaultContractResolver.GetClrTypeFullName(method.DeclaringType), method));
			if (method.IsVirtual)
				throw new JsonException("Virtual Method '{0}' of type '{1}' cannot be marked with '{2}' attribute.".FormatWith(
				   CultureInfo.InvariantCulture, method,
				   DefaultContractResolver.GetClrTypeFullName(method.DeclaringType), attributeType));
		...
		internal static string GetClrTypeFullName(Type type)
		{
			...
			return "{0}.{1}".FormatWith(CultureInfo.InvariantCulture, type.Namespace,
			   type.Name);
		}
		  ...
		  MemberSerialization memberSerialization)
		{
			...
			return source.OrderBy<JsonProperty, int>(p => p.Order ?? -1)
			  .ToList<JsonProperty>();
		}
		  ...
		  MemberSerialization memberSerialization)
		{
			...
			bool allowNonPublicAccess;
			this.SetPropertySettingsFromAttributes(property, member, member.Name, member.DeclaringType,
		  ...
		  out bool allowNonPublicAccess)
		{
			...
			JsonContainerAttribute attribute3 =
			  JsonTypeReflector.GetAttribute<JsonContainerAttribute>(declaringType);
			...
			bool flag2 = JsonTypeReflector.GetAttribute<JsonIgnoreAttribute>(attributeProvider) is object ||
		...
		private Predicate<object> CreateShouldSerializeTest(MemberInfo member)
		{
			...
			MethodCall<object, object> shouldSerializeCall =
			  JsonTypeReflector.ReflectionDelegateFactory.CreateMethodCall<object>(method);
		  ...
		  bool allowNonPublicAccess)
		{
			MemberInfo memberInfo = (MemberInfo)member.DeclaringType.GetProperty(member.Name + "Specified") ??
									 member.DeclaringType.GetField(member.Name + "Specified");
