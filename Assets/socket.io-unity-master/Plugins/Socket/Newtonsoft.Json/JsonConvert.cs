...
namespace Socket.Newtonsoft.Json
{
	public static class JsonConvert
	{
		  ...
		  DateTimeZoneHandling timeZoneHandling)
		{
			...
			{
				stringWriter.Write('"');
				DateTimeUtils.WriteDateTimeString(stringWriter, dateTime, format, null,
		  ...
		  bool nullable)
		{
			return JsonConvert.EnsureFloatFormat((double)value,
			  JsonConvert.EnsureDecimalPlace((double)value,
				value.ToString("R", CultureInfo.InvariantCulture)), floatFormatHandling, quoteChar,
			  nullable);
		}
		  ...
		  bool nullable)
		{
			return JsonConvert.EnsureFloatFormat(value,
			  JsonConvert.EnsureDecimalPlace(value, value.ToString("R", CultureInfo.InvariantCulture)),
		...
		{
			return JsonConvert.EnsureDecimalPlace(value.ToString(null,
			   CultureInfo.InvariantCulture));
		}
		...
		{
			string str1 = value.ToString("D", CultureInfo.InvariantCulture);
		...
		{
			return JsonConvert.SerializeObject(value, null, (JsonSerializerSettings)null);
		}
