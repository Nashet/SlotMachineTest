...
namespace Socket.Newtonsoft.Json
{
	public class JsonTextReader : JsonReader, IJsonLineInfo
	{
					...
					this.SetToken(JsonToken.Bytes,
					  this._stringReference.Length != 0
						? (this._stringReference.Length != 36 ||
						  ...
						  ? Convert.FromBase64CharArray(this._stringReference.Chars, this._stringReference.StartIndex,
							this._stringReference.Length)
						  : (object)g.ToByteArray())
						: CollectionUtils.ArrayEmpty<byte>(), false);
					break;
		...
		public override bool Read()
		{
				...
				if (this._chars[this._charPos] != '/')
					throw JsonReaderException.Create(this,
					  "Additional text encountered after finished reading JSON content: {0}.".FormatWith(
						 CultureInfo.InvariantCulture, this._chars[this._charPos]));
			...
			label_13:
			throw JsonReaderException.Create(this,
			  "Unexpected state: {0}.".FormatWith(CultureInfo.InvariantCulture,
				 CurrentState));
		}
		...
		private void ReadStringIntoBuffer(char quote)
		{
			...
			} while (this._chars[charPos1 - 1] != quote);
			...
			if (!ConvertUtils.TryHexTextToInt(this._chars, this._charPos, this._charPos + 4, out num1))
				throw JsonReaderException.Create(this,
				  "Invalid Unicode escape sequence: \\u{0}.".FormatWith(CultureInfo.InvariantCulture,
					 new string(this._chars, this._charPos, 4)));
			int num2 = Convert.ToChar(num1);
			this._charPos += 4;
			return (char)num2;
		}
			...
			if (this._chars[this._charPos] != ':')
				throw JsonReaderException.Create(this,
				  "Invalid character after parsing property name. Expected ':' but got: {0}.".FormatWith(
					 CultureInfo.InvariantCulture, this._chars[this._charPos]));
			++this._charPos;
			this.SetToken(JsonToken.PropertyName, str);
			this._quoteChar = quote;
			...
			switch (readType)
			{
				case ReadType.ReadAsInt32:
					...
					else if (flag2)
					{
						...
						try
						{
							obj = str.StartsWith("0x", StringComparison.OrdinalIgnoreCase)
							...
							case ParseResult.Overflow:
								throw this.ThrowReaderError(
								  "JSON integer {0} is too large or small for an Int32.".FormatWith(
									 CultureInfo.InvariantCulture, this._stringReference.ToString()),
								   null);
							default:
								throw this.ThrowReaderError(
				...
				case ReadType.ReadAsString:
					...
					else
					{
						...
						if (!double.TryParse(s, NumberStyles.Float, CultureInfo.InvariantCulture, out result))
							throw this.ThrowReaderError(
							  "Input string '{0}' is not a valid number.".FormatWith(CultureInfo.InvariantCulture,
								 this._stringReference.ToString()), null);
					}
				...
				case ReadType.ReadAsDecimal:
					if (flag1)
						obj = firstChar - new Decimal(48);
					else if (flag2)
					{
						string str = this._stringReference.ToString();
						try
						{
							obj = Convert.ToDecimal(str.StartsWith("0x", StringComparison.OrdinalIgnoreCase)
				...
				case ReadType.ReadAsDouble:
					...
					else if (flag2)
					{
						...
						try
						{
							obj = Convert.ToDouble(str.StartsWith("0x", StringComparison.OrdinalIgnoreCase)
							  ? Convert.ToInt64(str, 16)
							  : Convert.ToInt64(str, 8));
						}
						catch (Exception ex)
		...
		private void EndComment(bool setToken, int initialPosition, int endPosition)
		{
			...
			this.SetToken(JsonToken.Comment,
			   new string(this._chars, initialPosition, endPosition - initialPosition));
		}
		...
		private bool MatchValue(bool enoughChars, string value)
		{
			...
			{
				if (this._chars[this._charPos + index] != value[index])
				{
					this._charPos += index;
					return false;
				}
