...
namespace Socket.Newtonsoft.Json.Utilities
{
	internal static class ReflectionUtils
	{
		...
		public static ConstructorInfo GetDefaultConstructor(Type t, bool nonPublic)
		{
			...
			return t.GetConstructors(bindingAttr).SingleOrDefault<ConstructorInfo>(
			   c => !c.GetParameters().Any<ParameterInfo>());
		}
		  ...
		  out Type implementingType)
		{
			...
			if (!genericInterfaceDefinition.IsInterface() || !genericInterfaceDefinition.IsGenericTypeDefinition())
				throw new ArgumentNullException("'{0}' is not a generic interface definition.".FormatWith(
				   CultureInfo.InvariantCulture, genericInterfaceDefinition));
		...
		public static Type GetCollectionItemType(Type type)
		{
			...
			if (ReflectionUtils.ImplementsGenericDefinition(type, typeof(IEnumerable<>), out implementingType))
			{
				if (implementingType.IsGenericTypeDefinition())
					throw new Exception("Type {0} is not a collection.".FormatWith(CultureInfo.InvariantCulture,
					   type));
			...
			if (typeof(IEnumerable).IsAssignableFrom(type))
				return null;
			throw new Exception(
			  "Type {0} is not a collection.".FormatWith(CultureInfo.InvariantCulture, type));
		}
		  ...
		  out Type valueType)
		{
			...
			{
				if (implementingType.IsGenericTypeDefinition())
					throw new Exception("Type {0} is not a dictionary.".FormatWith(CultureInfo.InvariantCulture,
					   dictionaryType));
		  ...
		  BindingFlags bindingAttr)
		{
			   ...
			   m => m.Name))
			{
				...
				{
					IList<MemberInfo> memberInfoList3 = new List<MemberInfo>();
					...
					memberInfoList2.AddRange(memberInfoList3);
				}
			}
		  ...
		  bool inherit)
		{
			...
			if (type != null)
			{
				...
				if (inherit && type.BaseType != null)
					array = array
					  .Union<Attribute>(
						 ReflectionUtils.GetAttributes(type.BaseType, attributeType, inherit))
					  .ToArray<Attribute>();
				return array;
			}
		...
		{
			if (memberInfo.MemberType() != MemberTypes.Property)
				return targetType.GetMember(memberInfo.Name, memberInfo.MemberType(),
					BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
				  .SingleOrDefault<MemberInfo>();
			PropertyInfo propertyInfo = (PropertyInfo)memberInfo;
			Type[] array = propertyInfo.GetIndexParameters()
			  .Select<ParameterInfo, Type>(p => p.ParameterType).ToArray<Type>();
			return targetType.GetProperty(propertyInfo.Name,
			  BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null,
			  propertyInfo.PropertyType, array, null);
		}
		  ...
		  BindingFlags bindingAttr)
		{
			...
			ReflectionUtils.GetChildPrivateFields(source, targetType, bindingAttr);
		  ...
		  BindingFlags bindingAttr)
		{
			...
			while ((targetType = targetType.BaseType()) != null)
			{
				IEnumerable<FieldInfo> fieldInfos =
				   targetType.GetFields(bindingAttr1).Where<FieldInfo>(
					 f => f.IsPrivate);
		  ...
		  BindingFlags bindingAttr)
		{
			...
			List<PropertyInfo> propertyInfoList =
			  new List<PropertyInfo>(targetType.GetProperties(bindingAttr));
			if (targetType.IsInterface())
			{
				foreach (Type type in targetType.GetInterfaces())
					propertyInfoList.AddRange(type.GetProperties(bindingAttr));
			}

			ReflectionUtils.GetChildPrivateProperties(propertyInfoList, targetType, bindingAttr);
			for (int index = 0; index < propertyInfoList.Count; ++index)
			{
				...
				if (propertyInfo.DeclaringType != targetType)
				{
					PropertyInfo memberInfoFromType =
					  (PropertyInfo)ReflectionUtils.GetMemberInfoFromType(propertyInfo.DeclaringType, propertyInfo);
			...
			while ((targetType = targetType.BaseType()) != null)
			{
				foreach (PropertyInfo property in targetType.GetProperties(bindingAttr))
				{
					...
					else
					{
						...
						if (initialProperties.IndexOf<PropertyInfo>(p =>
						{
