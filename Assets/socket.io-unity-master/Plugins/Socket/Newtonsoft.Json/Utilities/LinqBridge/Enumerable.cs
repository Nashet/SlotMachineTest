	...
	internal static class Enumerable
	{
		  ...
		  Func<TSource, TCollection, TResult> resultSelector)
		{
			...
			return source.SelectMany<TSource, TCollection, TResult>(
			   (item, i) => collectionSelector(item), resultSelector);
		}
		...
		{
			Enumerable.CheckNotNull<Func<TSource, bool>>(predicate, nameof(predicate));
			return source.TakeWhile<TSource>((item, i) => predicate(item));
		}
		...
		public static TSource ElementAt<TSource>(this IEnumerable<TSource> source, int index)
		{
			...
			try
			{
				return source.SkipWhile<TSource>((item, i) => i < index).First<TSource>();
			}
		...
		public static TSource ElementAtOrDefault<TSource>(this IEnumerable<TSource> source, int index)
		{
			...
			IList<TSource> sourceList = source as IList<TSource>;
			if (sourceList == null)
				return source.SkipWhile<TSource>((item, i) => i < index).FirstOrDefault<TSource>();
		...
		public static IEnumerable<TSource> Take<TSource>(
		  ...
		  int count)
		{
			return source.Where<TSource>((item, i) => i < count);
		}
		...
		{
			Enumerable.CheckNotNull<Func<TSource, bool>>(predicate, nameof(predicate));
			return source.SkipWhile<TSource>((item, i) => predicate(item));
		}
		...
		public static long LongCount<TSource>(this IEnumerable<TSource> source)
		{
			...
			if (array == null)
				return source.Aggregate<TSource, long>(0L, (count, item) => count + 1L);
		...
		{
			return source.ToLookup<TSource, TKey, TSource>(keySelector, e => e,
			   null);
		}
		...
		public static IEnumerable<IGrouping<TKey, TSource>> GroupBy<TSource, TKey>(
		  ...
		  IEqualityComparer<TKey> comparer)
		{
			return source.GroupBy<TSource, TKey, TSource>(keySelector, e => e, comparer);
		}
		  ...
		  IEqualityComparer<TKey> comparer)
		{
			...
			Enumerable.CheckNotNull<Func<TSource, TKey>>(keySelector, nameof(keySelector));
			Enumerable.CheckNotNull<Func<TSource, TElement>>(elementSelector, nameof(elementSelector));
			return source.ToLookup<TSource, TKey, TElement>(keySelector,
		...
		public static IEnumerable<TSource> DefaultIfEmpty<TSource>(
		  this IEnumerable<TSource> source)
		{
			return source.DefaultIfEmpty<TSource>(default);
		}
		...
		{
			Enumerable.CheckNotNull<IEnumerable<TSource>>(source, nameof(source));
			return source.Aggregate<TSource>((a, item) =>
			{
				if (!lesser(a, item))
					return item;
				return a;
		...
		{
			Enumerable.CheckNotNull<IEnumerable<TSource?>>(source, nameof(source));
			return source.Aggregate<TSource?, TSource?>(seed, (a, item) =>
			{
				if (!lesser(a, item))
					return item;
				return a;
		  ...
		  bool flag)
		{
			...
			foreach (TSource key in first.Where<TSource>(k => !flags.ContainsKey(k)))
			{
				flags.Add(key, !flag);
				source.Add(key);
			}
			...
			foreach (TSource index in second.Where<TSource>(new Func<TSource, bool>(flags.ContainsKey)))
				flags[index] = flag;
			return source.Where<TSource>(item => flags[item]);
		}
		...
		public static Dictionary<TKey, TSource> ToDictionary<TSource, TKey>(
		  ...
		  IEqualityComparer<TKey> comparer)
		{
			return source.ToDictionary<TSource, TKey, TSource>(keySelector, e => e);
		}
		  ...
		  IEqualityComparer<TKey> comparer)
		{
			...
			return outer.Select<TOuter, TResult>(
			   o => resultSelector(o, lookup[outerKeySelector(o)]));
		}
		...
		public static double? Average(this IEnumerable<int?> source)
		{
			...
			long num1 = 0;
			long num2 = 0;
			foreach (int? nullable in source.Where<int?>(n => n.HasValue))
			{
				checked
				{
					num1 += nullable.Value;
		...
		{
			Enumerable.CheckNotNull<IEnumerable<int?>>(source, nameof(source));
			return source.Where<int?>(x => x.HasValue).MinMaxImpl<int>(new int?(),
			   (min, x) =>
			   {
				   int? nullable1 = min;
				   int? nullable2 = x;
		...
		public static double? Average(this IEnumerable<long?> source)
		{
			...
			long num1 = 0;
			long num2 = 0;
			foreach (long? nullable in source.Where<long?>(n => n.HasValue))
			{
				checked
				{
					num1 += nullable.Value;
		...
		{
			Enumerable.CheckNotNull<IEnumerable<long?>>(source, nameof(source));
			return source.Where<long?>(x => x.HasValue).MinMaxImpl<long>(new long?(),
			   (min, x) =>
			   {
				   long? nullable1 = min;
				   long? nullable2 = x;
		...
		public static float? Average(this IEnumerable<float?> source)
		{
			...
			float num1 = 0.0f;
			long num2 = 0;
			foreach (float? nullable in source.Where<float?>(n => n.HasValue))
			{
				num1 += nullable.Value;
				checked
				{
		...
		{
			Enumerable.CheckNotNull<IEnumerable<float?>>(source, nameof(source));
			return source.Where<float?>(x => x.HasValue).MinMaxImpl<float>(new float?(),
			   (min, x) =>
			   {
				   float? nullable1 = min;
				   float? nullable2 = x;
		...
		public static double? Average(this IEnumerable<double?> source)
		{
			...
			double num1 = 0.0;
			long num2 = 0;
			foreach (double? nullable in source.Where<double?>(n => n.HasValue))
			{
				num1 += nullable.Value;
				checked
				{
		...
		{
			Enumerable.CheckNotNull<IEnumerable<double?>>(source, nameof(source));
			return source.Where<double?>(x => x.HasValue).MinMaxImpl<double>(new double?(),
			   (min, x) =>
			   {
		...
		public static Decimal? Average(this IEnumerable<Decimal?> source)
		{
			...
			Decimal num1 = new Decimal();
			long num2 = 0;
			foreach (Decimal? nullable in source.Where<Decimal?>(n => n.HasValue))
			{
				num1 += nullable.Value;
				checked
				{
		...
		{
			Enumerable.CheckNotNull<IEnumerable<Decimal?>>(source, nameof(source));
			return source.Where<Decimal?>(x => x.HasValue).MinMaxImpl<Decimal>(new Decimal?(),
			   (min, x) =>
			   {
