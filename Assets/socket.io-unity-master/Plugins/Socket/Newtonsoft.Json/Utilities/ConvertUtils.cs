	  ...
	  new TypeInformation() {
		...
		t.InitialType
		  ...
		  t.InitialType
			...
			return o => call(null, new object[1] {
		o
	  });
		}
			...
			switch (ConvertUtils.TryConvertInternal(initialValue, culture, targetType, out obj))
			{
				...
				case ConvertUtils.ConvertResult.CannotConvertNull:
					throw new Exception("Can not convert null {0} into non-nullable {1}.".FormatWith(
					   CultureInfo.InvariantCulture, initialValue.GetType(), targetType));
		  ...
		  out object value)
		{
			...
			{
				value = converter1.ConvertTo(null, culture, initialValue, targetType);
				return ConvertUtils.ConvertResult.Success;
			}
		...
		private static object EnsureTypeAssignable(object value, Type initialType, Type targetType)
		{
			...
			throw new ArgumentException("Could not cast or convert from {0} to {1}.".FormatWith(
			   CultureInfo.InvariantCulture, initialType?.ToString() ?? "{null}",
			   targetType));
		}
		...
		public static object ToValue(INullable nullableValue)
		{
			...
			if (nullableValue is SqlDateTime)
				return ConvertUtils.ToValue(nullableValue);
			throw new ArgumentException(
			  "Unsupported INullable type: {0}".FormatWith(CultureInfo.InvariantCulture,
		  ...
		  out Decimal value)
		{
			...
			for (; index < num1; ++index)
			{
				...
				if (ch1 != '.')
				{
					...
					if (num7 < 29)
					{
						...
						if (num7 < 19)
							num5 = num5 * 10UL + (ulong)(ch1 - 48);
						else
							num6 = num6 * 10UL + (ulong)(ch1 - 48);
						++num7;
						continue;
					}
			...
			value = num7 > 19 ? num5 * ConvertUtils.DecimalFactors[num7 - 20] + num6 : num5;
			if (num10 > 0)
			{
				int num9 = num7 + num10;
				if (num9 > 29)
