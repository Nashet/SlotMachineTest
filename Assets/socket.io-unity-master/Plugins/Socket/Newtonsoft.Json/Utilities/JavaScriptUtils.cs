...
namespace Socket.Newtonsoft.Json.Utilities
{
	internal static class JavaScriptUtils
	{
		...
		{
			IList<char> first = new List<char>() {
		'\n',
		'\r',
		'\t',
		'\\',
		...
		'\b'
			...
			foreach (char ch in first.Union<char>(new char[1] {
		'\''
	  }))
				JavaScriptUtils.SingleQuoteCharEscapeFlags[ch] = true;
			foreach (char ch in first.Union<char>(new char[1] {
		'"'
	  }))
				JavaScriptUtils.DoubleQuoteCharEscapeFlags[ch] = true;
			foreach (char ch in first.Union<char>(new char[5] {
		'"',
		'\'',
		'<',
		'>',
		'&'
				...
				JavaScriptUtils.HtmlCharEscapeFlags[ch] = true;
		}
								...
								if (a != null)
								{
									...
									if (index > num1)
									{
										...
										if (writeBuffer == null || writeBuffer.Length < minSize)
										{
											...
											if (flag)
												Array.Copy(writeBuffer, chArray, 6);
		  ...
		  StringEscapeHandling stringEscapeHandling)
		{
			...
			using (StringWriter stringWriter = StringUtils.CreateStringWriter(value != null ? value.Length : 16))
			{
				...
				JavaScriptUtils.WriteEscapedJavaScriptString(stringWriter, value, delimiter, appendDelimiters,
				  charEscapeFlags, stringEscapeHandling, null, ref writeBuffer);
		  ...
		  StringEscapeHandling stringEscapeHandling)
		{
			...
			{
				char ch = s[index];
				if (ch < charEscapeFlags.Length)
				{
					if (charEscapeFlags[ch])
						return index;
				}
