...
namespace Socket.Newtonsoft.Json.Utilities
{
	internal class BidirectionalDictionary<TFirst, TSecond>
	{
		...
		public BidirectionalDictionary()
		  : this(EqualityComparer<TFirst>.Default,
		...
		{
			TSecond second1;
			if (this._firstToSecond.TryGetValue(first, out second1) && !second1.Equals(second))
				throw new ArgumentException(
				  this._duplicateFirstErrorMessage.FormatWith(CultureInfo.InvariantCulture, first));
			TFirst first1;
			if (this._secondToFirst.TryGetValue(second, out first1) && !first1.Equals(first))
				throw new ArgumentException(
				  this._duplicateSecondErrorMessage.FormatWith(CultureInfo.InvariantCulture,
					 second));
