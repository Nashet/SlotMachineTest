...
namespace Socket.Newtonsoft.Json.Schema
{
	...
	internal class JsonSchemaBuilder
	{
			  ...
			  '/'
					...
					if (jsonSchema == null)
						throw new JsonException("Could not resolve schema reference '{0}'.".FormatWith(
						   CultureInfo.InvariantCulture, schema.DeferredReference));
				}
		...
		{
			IList<JsonSchema> jsonSchemaList = new List<JsonSchema>();
			if (token.Type == JTokenType.Array)
			{
				foreach (JToken token1 in token)
				...
				if (dictionary.ContainsKey(jproperty.Name))
					throw new JsonException(
					  "Property {0} has already been defined in schema.".FormatWith(
						 CultureInfo.InvariantCulture, jproperty.Name));
				...
				case JTokenType.Array:
					...
					foreach (JToken jtoken in token)
					{
						if (jtoken.Type != JTokenType.String)
							throw JsonException.Create(jtoken, jtoken.Path,
