...
namespace Socket.Newtonsoft.Json.Schema
{
	...
	internal class JsonSchemaModelBuilder
	{
		...
		public JsonSchemaNode AddSchema(JsonSchemaNode existingNode, JsonSchema schema)
		{
			...
			if (existingNode != null)
			{
				if (existingNode.Schemas.Contains(schema))
					return existingNode;
				id = JsonSchemaNode.GetId(existingNode.Schemas.Union<JsonSchema>(new JsonSchema[1]
				{
		  schema
				}));
			}
		...
		{
			JsonSchemaNode jsonSchemaNode = this.AddSchema(parentNode.Items.Count > index ? parentNode.Items[index] : null, schema);
		...
		private JsonSchemaModel BuildNodeModel(JsonSchemaNode node)
		{
			...
			JsonSchemaModel jsonSchemaModel2 = JsonSchemaModel.Create(node.Schemas);
			...
			foreach (KeyValuePair<string, JsonSchemaNode> property in node.Properties)
			{
				if (jsonSchemaModel2.Properties == null)
					jsonSchemaModel2.Properties = new Dictionary<string, JsonSchemaModel>();
			...
			foreach (KeyValuePair<string, JsonSchemaNode> patternProperty in node.PatternProperties)
			{
				if (jsonSchemaModel2.PatternProperties == null)
					jsonSchemaModel2.PatternProperties = new Dictionary<string, JsonSchemaModel>();
			...
			foreach (JsonSchemaNode node1 in node.Items)
			{
				if (jsonSchemaModel2.Items == null)
					jsonSchemaModel2.Items = new List<JsonSchemaModel>();
