	   ...
	   244
		...
		private void ReadType(BsonType type)
		{
			switch (type)
			{
				case BsonType.Number:
					...
					if (this._floatParseHandling == FloatParseHandling.Decimal)
					{
						this.SetToken(JsonToken.Float,
						   Convert.ToDecimal(num, CultureInfo.InvariantCulture));
						break;
					}
				...
				case BsonType.Binary:
					...
					this.SetToken(JsonToken.Bytes, binaryType != BsonBinaryType.Uuid ? b : (object)new Guid(b));
					break;
				...
				case BsonType.Boolean:
					this.SetToken(JsonToken.Boolean, Convert.ToBoolean(this.ReadByte()));
					break;
				case BsonType.Date:
				...
				case BsonType.Null:
					...
					break;
				case BsonType.Regex:
					this.SetToken(JsonToken.String, "/" + this.ReadString() + "/" + this.ReadString());
					break;
		...
		private string ReadString()
		{
			...
			while (true)
			{
				int num2 = length;
				byte num3;
				while (num2 < 128 && (num3 = this._reader.ReadByte()) > 0)
				...
				if (num2 >= 128 || stringBuilder != null)
				{
					...
					if (lastFullCharStop < byteCount - 1)
					{
						...
						Array.Copy(_byteBuffer, lastFullCharStop + 1, _byteBuffer, 0, length);
					}
					else if (num2 >= 128)
						length = 0;
					else
		...
		private int BytesInSequence(byte b)
		{
			...
			if (b >= BsonReader.SeqRange3[0] && b <= BsonReader.SeqRange3[1])
				return 3;
			return b >= BsonReader.SeqRange4[0] && b <= BsonReader.SeqRange4[1] ? 4 : 0;
		}
