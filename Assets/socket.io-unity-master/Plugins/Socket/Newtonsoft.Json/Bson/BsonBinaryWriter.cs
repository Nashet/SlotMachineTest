		...
		private void WriteTokenInternal(BsonToken t)
		{
			switch (t.Type)
			{
				case BsonType.Number:
					this._writer.Write(Convert.ToDouble(((BsonValue)t).Value, CultureInfo.InvariantCulture));
					break;
				...
				case BsonType.Array:
					...
					foreach (BsonToken t1 in bsonArray)
					{
						...
						this.WriteString(i.ToString(CultureInfo.InvariantCulture), MathUtils.IntLength(i), new int?());
				...
				case BsonType.Integer:
					this._writer.Write(Convert.ToInt32(((BsonValue)t).Value, CultureInfo.InvariantCulture));
					break;
				case BsonType.Long:
					this._writer.Write(Convert.ToInt64(((BsonValue)t).Value, CultureInfo.InvariantCulture));
					break;
				default:
		...
		private int CalculateSize(BsonToken t)
		{
			switch (t.Type)
			{
				...
				case BsonType.Object:
					...
					foreach (BsonProperty bsonProperty in bsonObject)
					{
						int num2 = 1 + this.CalculateSize(bsonProperty.Name) + this.CalculateSize(bsonProperty.Value);
						num1 += num2;
					}
					int num3 = num1 + 1;
				...
				case BsonType.Regex:
					BsonRegex bsonRegex = (BsonRegex)t;
					int num6 = 0 + this.CalculateSize(bsonRegex.Pattern) + this.CalculateSize(bsonRegex.Options);
