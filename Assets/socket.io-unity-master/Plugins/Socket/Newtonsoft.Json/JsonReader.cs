...
namespace Socket.Newtonsoft.Json
{
	public abstract class JsonReader : IDisposable
	{
				...
				case JsonToken.Float:
					if (!(this.Value is int))
						this.SetToken(JsonToken.Integer,
						   Convert.ToInt32(this.Value, CultureInfo.InvariantCulture), false);
					return new int?((int)this.Value);
				case JsonToken.String:
					return this.ReadInt32String((string)this.Value);
				default:
					throw JsonReaderException.Create(this,
					  "Error reading integer. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
						 contentToken));
			}
		}
		...
		internal int? ReadInt32String(string s)
		{
			...
			{
				this.SetToken(JsonToken.Integer, result, false);
				return new int?(result);
			}

			this.SetToken(JsonToken.String, s, false);
			throw JsonReaderException.Create(this,
			  "Could not convert string to integer: {0}.".FormatWith(CultureInfo.InvariantCulture,
				 s));
		}
			...
			switch (contentToken)
			{
				...
				default:
					if (!JsonTokenUtils.IsPrimitiveToken(contentToken) || this.Value == null)
						throw JsonReaderException.Create(this,
						  "Error reading string. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
							 contentToken));
					IFormattable formattable = this.Value as IFormattable;
					string str;
					if (formattable != null)
					{
						str = formattable.ToString(null, Culture);
					}
					else
					{
						Uri uri = this.Value as Uri;
						str = uri != null ? uri.OriginalString : this.Value.ToString();
					}
		...
		public virtual byte[] ReadAsBytes()
		{
			...
			switch (contentToken)
			{
				...
				case JsonToken.String:
					...
					this.SetToken(JsonToken.Bytes, numArray2, false);
					return numArray2;
				...
				case JsonToken.Float:
					if (!(this.Value is double))
						this.SetToken(JsonToken.Float,
						   Convert.ToDouble(this.Value, CultureInfo.InvariantCulture), false);
					return new double?((double)this.Value);
				case JsonToken.String:
					return this.ReadDoubleString((string)this.Value);
				default:
					throw JsonReaderException.Create(this,
					  "Error reading double. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
						 contentToken));
			}
		}
		...
		internal double? ReadDoubleString(string s)
		{
			...
			double result;
			if (double.TryParse(s, NumberStyles.Float | NumberStyles.AllowThousands, Culture,
			  out result))
			{
				this.SetToken(JsonToken.Float, result, false);
				return new double?(result);
			}

			this.SetToken(JsonToken.String, s, false);
			throw JsonReaderException.Create(this,
			  "Could not convert string to double: {0}.".FormatWith(CultureInfo.InvariantCulture,
				 s));
		}
		...
		public virtual bool? ReadAsBoolean()
		{
			...
			switch (contentToken)
			{
				...
				case JsonToken.Integer:
				case JsonToken.Float:
					bool boolean = Convert.ToBoolean(this.Value, CultureInfo.InvariantCulture);
					this.SetToken(JsonToken.Boolean, boolean, false);
				...
				default:
					throw JsonReaderException.Create(this,
					  "Error reading boolean. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
						 contentToken));
			}
		}
		...
		internal bool? ReadBooleanString(string s)
		{
			...
			{
				this.SetToken(JsonToken.Boolean, result, false);
				return new bool?(result);
			}

			this.SetToken(JsonToken.String, s, false);
			throw JsonReaderException.Create(this,
			  "Could not convert string to boolean: {0}.".FormatWith(CultureInfo.InvariantCulture,
				 s));
		}
			...
			switch (contentToken)
			{
				...
				case JsonToken.Float:
					if (!(this.Value is Decimal))
						this.SetToken(JsonToken.Float,
						   Convert.ToDecimal(this.Value, CultureInfo.InvariantCulture), false);
					return new Decimal?((Decimal)this.Value);
				case JsonToken.String:
					return this.ReadDecimalString((string)this.Value);
				default:
					throw JsonReaderException.Create(this,
					  "Error reading decimal. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
						 contentToken));
			}
		}
		...
		internal Decimal? ReadDecimalString(string s)
		{
			...
			this.SetToken(JsonToken.String, s, false);
			throw JsonReaderException.Create(this,
			  "Could not convert string to decimal: {0}.".FormatWith(CultureInfo.InvariantCulture,
				 s));
		}
		...
		public virtual DateTime? ReadAsDateTime()
		{
			switch (this.GetContentToken())
			{
				...
				default:
					throw JsonReaderException.Create(this,
					  "Error reading date. Unexpected token: {0}.".FormatWith(CultureInfo.InvariantCulture,
						 TokenType));
			}
		}
		...
		internal DateTime? ReadDateTimeString(string s)
		{
			...
			if (!DateTime.TryParse(s, Culture, DateTimeStyles.RoundtripKind, out dateTime))
				throw JsonReaderException.Create(this,
				  "Could not convert string to DateTime: {0}.".FormatWith(CultureInfo.InvariantCulture,
					 s));
			dateTime = DateTimeUtils.EnsureDateTime(dateTime, this.DateTimeZoneHandling);
			this.SetToken(JsonToken.Date, dateTime, false);
			return new DateTime?(dateTime);
			...
			if (this.GetTypeForCloseToken(endToken) != jsonContainerType)
				throw JsonReaderException.Create(this,
				  "JsonToken {0} is not valid for closing JsonType {1}.".FormatWith(
					 CultureInfo.InvariantCulture, endToken, jsonContainerType));
			if (this.Peek() != JsonContainerType.None)
				this._currentState = JsonReader.State.PostValue;
			else
				this.SetFinished();
				...
				default:
					throw JsonReaderException.Create(this,
					  "While setting the reader state back to current object an unexpected JsonType was encountered: {0}"
						.FormatWith(CultureInfo.InvariantCulture, jsonContainerType));
			}
		}
		...
		private JsonContainerType GetTypeForCloseToken(JsonToken token)
		{
			switch (token)
			{
				...
				default:
					throw JsonReaderException.Create(this,
					  "Not a valid close JsonToken: {0}".FormatWith(CultureInfo.InvariantCulture,
						 token));
			}
		}
