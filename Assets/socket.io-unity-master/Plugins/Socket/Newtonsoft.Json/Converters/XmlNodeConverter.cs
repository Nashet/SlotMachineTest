...
namespace Socket.Newtonsoft.Json.Converters
{
	public class XmlNodeConverter : JsonConverter
	{
			...
			switch (node.ChildNodes.Count)
			{
				...
				default:
						...
						if (dictionary == null)
						{
							...
							else if (!(propertyName2 == str))
							{
								...
								if (capacity > 1)
								{
									...
									dictionary.Add(str, xmlNodeList);
								}
								else
									dictionary.Add(str, node.ChildNodes[0]);

								dictionary.Add(propertyName2, childNode);
							}
						}
						else
						{
					...
					(IXmlNode) obj
									...
									dictionary[propertyName2] = xmlNodeList;
								}
			...
			switch (reader.TokenType)
			{
				case JsonToken.StartObject:
					...
					if (typeof(XmlNode).IsAssignableFrom(objectType))
					{
						...
						document = new XmlDocumentWrapper(new XmlDocument()
						{
							XmlResolver = null
						});
					...
					if (document == null || currentNode == null)
						throw JsonSerializationException.Create(reader,
						  "Unexpected type when converting XML: " + objectType);
		  ...
		  XmlNamespaceManager manager)
		{
			...
			{
				this.DeserializeValue(reader, document, manager, propertyName, element1);
				++num;
			}
