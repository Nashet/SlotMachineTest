				...
				if (a != null)
				{
					...
					else if (errorWhenNoMatch)
					{
						...
						else
						{
							...
							str1 = valueOrDefault.ToString(CultureInfo.InvariantCulture);
						}
						...
						else
						{
							nullable = this.End;
							valueOrDefault = nullable.GetValueOrDefault();
							str2 = valueOrDefault.ToString(CultureInfo.InvariantCulture);
						}

						throw new JsonException(
						  "Array slice of {0} to {1} returned no results.".FormatWith(invariantCulture,
							 str1, str2));
					}
				}
				else if (errorWhenNoMatch)
