...
namespace Socket.Newtonsoft.Json.Linq.JsonPath
{
	internal class JPath
	{
				...
				else
				{
					...
					switch (c)
					{
						...
						case ',':
							...
							intList.Add(Convert.ToInt32(str, CultureInfo.InvariantCulture));
							++this._currentIndex;
							this.EatWhitespace();
		...
		private QueryExpression ParseExpression()
		{
			...
			CompositeExpression compositeExpression1 = null;
			while (this._currentIndex < this._expression.Length)
			{
				...
				if (this._expression[this._currentIndex] == ')')
				{
					...
					compositeExpression1.Expressions.Add(booleanQueryExpression2);
				...
				if (this._expression[this._currentIndex] == '&')
				{
					...
					if (compositeExpression1 == null || compositeExpression1.Operator != QueryOperator.And)
					{
						...
						compositeExpression1?.Expressions.Add(compositeExpression3);
						compositeExpression1 = compositeExpression3;
						if (queryExpression == null)
							queryExpression = compositeExpression1;
					}
			...
			for (++this._currentIndex; this._currentIndex < this._expression.Length; ++this._currentIndex)
			{
				...
				switch (ch)
				{
					...
					case ')':
						...
						long result1;
						int num1 = long.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture,
						  out result1)
						  ? 1
						  : 0;
						value = result1;
		...
		private bool Match(string s)
		{
			...
			{
				if (currentIndex >= this._expression.Length || this._expression[currentIndex] != ch)
					return false;
				++currentIndex;
			}
		...
		internal static IEnumerable<JToken> Evaluate(
		  ...
		  bool errorWhenNoMatch)
		{
			IEnumerable<JToken> current = new JToken[1] {
		t
	  };
