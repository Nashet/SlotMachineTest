	...
	public class JTokenReader : JsonReader, IJsonLineInfo
	{
		...
		private JsonToken? GetEndToken(JContainer c)
		{
			switch (c.Type)
			{
				...
				default:
					throw MiscellaneousUtils.CreateArgumentOutOfRangeException("Type", c.Type,
			...
			switch (token.Type)
			{
				...
				case JTokenType.Uri:
					...
					this.SetToken(JsonToken.String,
					  uri != null ? uri.OriginalString : (object)this.SafeToString(obj));
					break;
				case JTokenType.TimeSpan:
					this.SetToken(JsonToken.String, this.SafeToString(((JValue)token).Value));
					break;
				default:
					throw MiscellaneousUtils.CreateArgumentOutOfRangeException("Type", token.Type,
