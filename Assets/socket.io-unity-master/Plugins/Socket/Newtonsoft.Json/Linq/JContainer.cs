...
namespace Socket.Newtonsoft.Json.Linq
{
	...
	ITypedList, IBindingList, IList, ICollection
	{
		  ...
		  : this()
		{
			...
			{
				this.AddInternal(index, jtoken, false);
				++index;
			}
		}
			...
			if (this._busy)
				throw new InvalidOperationException(
				  "Cannot change {0} during a collection change event.".FormatWith(
					 CultureInfo.InvariantCulture, this.GetType()));
		}
		...
		internal virtual void InsertItem(int index, JToken item, bool skipParentCheck)
		{
			...
			JToken jtoken1 = index == 0 ? null : childrenTokens[index - 1];
		...
		{
			return content as JToken ?? new JValue(content);
		}
		...
		internal void ReadTokenFrom(JsonReader reader, JsonLoadSettings options)
		{
			...
			if (!reader.Read())
				throw JsonReaderException.Create(reader,
				  "Error reading {0} from JsonReader.".FormatWith(CultureInfo.InvariantCulture,
					 GetType().Name));
			...
			do
			{
				...
				switch (r.TokenType)
				{
					...
					case JsonToken.PropertyName:
						...
						if (jproperty2 == null)
							jcontainer.Add(jproperty1);
						else
							jproperty2.Replace(jproperty1);
						jcontainer = jproperty1;
						goto case JsonToken.None;
		...
		object IBindingList.AddNew()
		{
			...
			if (e.NewObject == null)
				throw new JsonException(
				  "Could not determine new value to add to '{0}'.".FormatWith(CultureInfo.InvariantCulture,
					 this.GetType()));
			...
			switch (settings.MergeArrayHandling)
			{
				...
				case MergeArrayHandling.Union:
					Dictionary<JToken, bool> dictionary =
					  new Dictionary<JToken, bool>(EqualityComparer);
					foreach (JToken index in target)
