	...
	public class JProperty : JContainer
	{
		...
		internal override bool RemoveItem(JToken item)
		{
			throw new JsonException(
			  "Cannot add or remove items from {0}.".FormatWith(CultureInfo.InvariantCulture,
				 typeof(JProperty)));
		}
		...
		internal override void InsertItem(int index, JToken item, bool skipParentCheck)
		{
			...
			if (this.Value != null)
				throw new JsonException(
				  "{0} cannot have multiple values.".FormatWith(CultureInfo.InvariantCulture,
					 typeof(JProperty)));
		...
		internal override bool DeepEquals(JToken node)
		{
			...
			if (jproperty != null && this._name == jproperty.Name)
				return this.ContentsEqual(jproperty);
			return false;
		}
		...
		public JProperty(string name, object content)
		{
			ValidationUtils.ArgumentNotNull(name, nameof(name));
			this._name = name;
			this.Value = this.IsMultiContent(content) ? new JArray(content) : JContainer.CreateFromContent(content);
		}
