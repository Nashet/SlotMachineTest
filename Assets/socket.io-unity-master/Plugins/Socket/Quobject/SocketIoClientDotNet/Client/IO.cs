...
namespace Socket.Quobject.SocketIoClientDotNet.Client
{
	public class IO
	{
		...
		public static QSocket Socket(Uri uri, IO.Options opts)
		{
			...
			{
				logger.Info(string.Format("ignoring socket cache for {0}", uri.ToString()));
				manager = new Manager(uri, opts);
			}
			else
			{
				...
				{
					logger.Info(string.Format("new io instance for {0}", id));
					IO.Managers.Add(id, new Manager(uri, opts));
				}
