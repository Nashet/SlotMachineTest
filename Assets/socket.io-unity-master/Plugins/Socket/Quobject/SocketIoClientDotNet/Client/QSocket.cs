	...
	public class QSocket : Emitter
	{
	  ...
	  QSocket.EVENT_RECONNECTING
		...
		private void SubEvents()
		{
			...
			this.Subs = this.Subs.Enqueue(ClientOn.Create(io, Manager.EVENT_OPEN,
			  new ListenerImpl(new ActionTrigger(this.OnOpen))));
			this.Subs = this.Subs.Enqueue(ClientOn.Create(io, Manager.EVENT_PACKET,
			  new ListenerImpl(data => this.OnPacket((Packet)data))));
			this.Subs = this.Subs.Enqueue(ClientOn.Create(io, Manager.EVENT_CLOSE,
		...
		public override Emitter Emit(string eventString, params object[] args)
		{
		 ...
		 eventString
			...
			objectList.AddRange(args);
			JArray a = Packet.Args2JArray(objectList);
			Packet packet = new Packet(HasBinaryData.HasBinary(a) ? 5 : 2, a);
			object obj = objectList[objectList.Count - 1];
			if (obj is IAck)
			{
				logger.Info(string.Format("emitting packet with ack id {0}", Ids));
		...
		public Emitter Emit(string eventString, IAck ack, params object[] args)
		{
		 ...
		 eventString
			...
			if (args != null)
				objectList.AddRange(args);
			Packet packet = new Packet(2, new JArray(objectList));
			logger.Info(string.Format("emitting packet with ack id {0}", Ids));
		...
		{
			return this.Emit(eventString, new AckImpl(ack), args);
		}
		...
		{
			LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format("close ({0})", reason));
			this.Connected = false;
			this.Emit(QSocket.EVENT_DISCONNECT, reason);
		}
		...
		private void OnEvent(Packet packet)
		{
			...
			logger.Info(string.Format("emitting event {0}", dataAsList));
			if (packet.Id >= 0)
			{
				logger.Info("attaching ack callback to event");
				dataAsList.Add(new QSocket.AckImp(this, packet.Id));
			}
		...
		private void OnAck(Packet packet)
		{
			LogManager.GetLogger(Global.CallerName("", 0, ""))
			  .Info(string.Format("calling ack {0} with {1}", packet.Id, packet.Data));
		...
		{
			LogManager.GetLogger(Global.CallerName("", 0, ""))
			  .Info(string.Format("server disconnect ({0})", Nsp));
			this.Destroy();
		...
		public QSocket Close()
		{
			...
			if (this.Connected)
			{
				logger.Info(string.Format("performing disconnect ({0})", Nsp));
		...
		private class AckImp : IAck
		{
			...
			public void Call(params object[] args)
			{
				...
				this.sent[0] = true;
				LogManager logger = LogManager.GetLogger(Global.CallerName("", 0, ""));
				JArray jarray = Packet.Args2JArray(args);
				logger.Info(string.Format("sending ack {0}", args.Length != 0 ? jarray.ToString() : (object)"null"));
				this.socket.Packet_method(new Packet(HasBinaryData.HasBinary(args) ? 6 : 3, jarray)
				{
					Id = this.Id
				});
			}
