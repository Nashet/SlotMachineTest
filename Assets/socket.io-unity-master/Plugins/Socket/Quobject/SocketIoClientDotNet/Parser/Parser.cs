...
namespace Socket.Quobject.SocketIoClientDotNet.Parser
{
	public class Parser
	{
	  ...
	  nameof(ERROR)
		...
		private static Packet ErrorPacket = new Packet(4, "parser error");
		...
		public class Encoder
		{
			...
			{
				LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format("encoding packet {0}", obj));
				...
				else
				{
					...
					callback.Call(new object[1] { str });
				}
			}
			...
			private string EncodeAsString(Packet obj)
			{
				...
				LogManager.GetLogger(Global.CallerName("", 0, ""))
				  .Info(string.Format("encoded {0} as {1}", obj, stringBuilder));
		...
		public class Decoder : Emitter
		{
			...
			private Packet decodeString(string str)
			{
				...
				string s1 = startIndex1 + 1 >= str.Length ? null : str.Substring(startIndex1 + 1, 1);
				int result;
				...
				LogManager.GetLogger(Global.CallerName("", 0, ""))
				  .Info(string.Format("decoded {0} as {1}", str, packet));
				return packet;
			}
