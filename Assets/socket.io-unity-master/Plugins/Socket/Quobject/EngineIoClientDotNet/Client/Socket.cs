...
namespace Socket.Quobject.EngineIoClientDotNet.Client
{
	public class Socket : Emitter
	{
		...
		public Socket(string uri, Socket.Options options)
		  : this(uri == null ? null : Socket.String2Uri(uri), options)
		{
		}
		...
		public Socket(Uri uri, Socket.Options options)
		  : this(uri == null ? options : Socket.Options.FromURI(uri, options))
		{
		}
		...
		{
			Dictionary<string, string> dictionary = new Dictionary<string, string>(Query);
		...
		{
			LogManager logger = LogManager.GetLogger(Global.CallerName("", 0, ""));
			logger.Info(string.Format("SetTransport setting transport '{0}'", transport.Name));
			if (this.mTransport != null)
			{
				logger.Info(string.Format("SetTransport clearing existing transport '{0}'", transport.Name));
				this.mTransport.Off();
			}

			this.mTransport = transport;
			this.Emit(Socket.EVENT_TRANSPORT, transport);
			transport.On(Socket.EVENT_DRAIN, new Socket.EventDrainListener(this));
			transport.On(Socket.EVENT_PACKET, new Socket.EventPacketListener(this));
			transport.On(Socket.EVENT_ERROR, new Socket.EventErrorListener(this));
			transport.On(Socket.EVENT_CLOSE, new Socket.EventCloseListener(this));
		}
		...
		public void OnPacket(Packet packet)
		{
			...
			{
				logger.Info(string.Format("socket received: type '{0}', data '{1}'", packet.Type, packet.Data));
				this.Emit(Socket.EVENT_PACKET, packet);
				...
				{
					this.Emit(Socket.EVENT_ERROR, new EngineIOException("server error")
					{
						code = packet.Data
					});
				}
				...
				logger.Info(string.Format("OnPacket packet received with socket readyState '{0}'", ReadyState));
		}

		private void OnHandshake(HandshakeData handshakeData)
		{
			LogManager.GetLogger(Global.CallerName("", 0, "")).Info(nameof(OnHandshake));
			this.Emit(Socket.EVENT_HANDSHAKE, handshakeData);
			this.Id = handshakeData.Sid;
			this.mTransport.Query.Add("sid", handshakeData.Sid);
			this.Upgrades = this.FilterUpgrades(handshakeData.Upgrades);
			...
			if (Socket.ReadyStateEnum.CLOSED == this.ReadyState)
				return;
			this.SetPing();
			this.Off(Socket.EVENT_HEARTBEAT, new Socket.OnHeartbeatAsListener(this));
			this.On(Socket.EVENT_HEARTBEAT, new Socket.OnHeartbeatAsListener(this));
		}
		...
		private void SetPing()
		{
			...
			this.PingIntervalTimer = EasyTimer.SetTimeout(() =>
			{
				LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format(
				  "writing ping packet - expecting pong within {0}ms, ReadyState = {1}", PingTimeout,
				   ReadyState));
				this.Ping();
		...
		{
			this.SendPacket(new Packet(type, data), fn);
		}
		...
		private void SendPacket(Packet packet, ActionTrigger fn)
		{
			if (fn == null)
				fn = () => { };
			this.Emit(Socket.EVENT_PACKET_CREATE, packet);
		...
		private void Probe(string name)
		{
			LogManager.GetLogger(Global.CallerName("", 0, ""))
			  .Info(string.Format("Probe probing transport '{0}'", name));
			...
			Socket.ProbingOnErrorListener onError =
			  new Socket.ProbingOnErrorListener(this, parameters.Transport, freezeTransport);
			...
			parameters.Cleanup = parameters.Cleanup.Add(() =>
			{
				...
				this.Off(Socket.EVENT_CLOSE, onClose);
				this.Off(Socket.EVENT_UPGRADING, onUpgrade);
			});
			...
			this.Once(Socket.EVENT_CLOSE, onClose);
			this.Once(Socket.EVENT_UPGRADING, onUpgrade);
		...
		public Socket Close()
		{
			if (this.ReadyState == Socket.ReadyStateEnum.OPENING || this.ReadyState == Socket.ReadyStateEnum.OPEN)
			{
				LogManager logger = LogManager.GetLogger(Global.CallerName("", 0, ""));
				logger.Info("Start");
				this.OnClose("forced close", null);
		...
		private void OnClose(string reason, Exception desc = null)
		{
			if (this.ReadyState != Socket.ReadyStateEnum.OPENING && this.ReadyState != Socket.ReadyStateEnum.OPEN)
				return;
			LogManager logger = LogManager.GetLogger(Global.CallerName("", 0, ""));
			logger.Info(string.Format("OnClose socket close with reason: {0}", reason));
			if (this.PingIntervalTimer != null)
				this.PingIntervalTimer.Stop();
			...
			this.Id = null;
			this.Emit(Socket.EVENT_CLOSE, reason, desc);
		}
		...
		internal void OnError(Exception exception)
		{
			...
			if (this._errorCount != 0)
				return;
			++this._errorCount;
			this.Emit(Socket.EVENT_ERROR, exception);
		...
		private class EventPacketListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				this.socket.OnPacket(args.Length != 0 ? (Packet)args[0] : null);
			}
		...
		private class EventErrorListener : IListener, IComparable<IListener>
		{
			...
			{
				this.socket.OnError(args.Length != 0 ? (Exception)args[0] : null);
			}
		...
		private class EventCloseListener : IListener, IComparable<IListener>
		{
			...
			public void Call(params object[] args)
			{
				this.socket.OnClose("transport close", null);
			}
		...
		private class OnTransportOpenListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				...
				this.Parameters.Transport[0].Once(Transport.EVENT_PACKET,
				   new Socket.OnTransportOpenListener.ProbeEventPacketListener(this));
				...
				void IListener.Call(params object[] args)
				{
					...
					if (Packet.PONG == packet.Type && "probe" == (string)packet.Data)
					{
						...
						this._onTransportOpenListener.Parameters.Socket.Emit(Socket.EVENT_UPGRADING,
						   this._onTransportOpenListener.Parameters.Transport[0]);
						Socket.PriorWebsocketSuccess = WebSocket.NAME == this._onTransportOpenListener.Parameters.Transport[0].Name;
						((Polling)this._onTransportOpenListener.Parameters.Socket.mTransport).Pause(() =>
						{
							if (this._onTransportOpenListener.Parameters.Failed[0] ||
		...
		private class ProbingOnErrorListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				...
				LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format(
				  "probe transport \"{0}\" failed because of error: {1}", engineIoException.Transport, obj));
				this._socket.Emit(Socket.EVENT_UPGRADE_ERROR, engineIoException);
			}
		...
		private class ProbingOnTransportCloseListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				this._onError.Call("transport closed");
			}
		...
		private class ProbingOnCloseListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				this._onError.Call("socket closed");
			}
		...
		private class ProbingOnUpgradeListener : IListener, IComparable<IListener>
		{
			...
			void IListener.Call(params object[] args)
			{
				...
				LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format("'{0}' works - aborting '{1}'",
				   transport.Name, _transport[0].Name));
				this._freezeTransport.Call();
			}
