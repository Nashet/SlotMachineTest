...
namespace Socket.Quobject.EngineIoClientDotNet.Client
{
	public abstract class Transport : Emitter
	{
		...
		public bool Writable
		{
			...
			set
			{
				LogManager.GetLogger(Global.CallerName("", 0, "")).Info(string.Format("Writable: {0} sid={1}", value, Socket.Id));
		...
		{
			Exception exception1 = new EngineIOException(message, exception);
			this.Emit(Transport.EVENT_ERROR, exception1);
			return this;
		}
		...
		{
			this.Emit(Transport.EVENT_PACKET, packet);
		}
		...
		{
			LogManager.GetLogger(Global.CallerName("", 0, "")).Info("Send called with packets.Count: " + packets.Count);
		...
		public class Options
		{
			...
			public string GetCookiesAsString()
			{
				...
				foreach (KeyValuePair<string, string> cookie in this.Cookies)
				{
					...
					stringBuilder.Append(string.Format("{0}={1}", cookie.Key, cookie.Value));
					flag = false;
				}
