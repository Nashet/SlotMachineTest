	...
	public class Polling : Transport
	{
		...
		public void Pause(ActionTrigger onPause)
		{
			...
			if (this.IsPolling || !this.Writable)
			{
				...
				{
					++total[0];
					this.Once(Polling.EVENT_POLL_COMPLETE, new Polling.PauseEventPollCompleteListener(total, pause));
				}
				if (this.Writable)
					return;
				++total[0];
				this.Once(Transport.EVENT_DRAIN, new Polling.PauseEventDrainListener(total, pause));
			}
			else
				pause();
		}
		...
		private void _onData(object data)
		{
			...
			if (data is string)
				Quobject.EngineIoClientDotNet.Parser.Parser2.DecodePayload((string)data, decodePayloadCallback);
			else if (data is byte[])
				Quobject.EngineIoClientDotNet.Parser.Parser2.DecodePayload((byte[])data, decodePayloadCallback);
			...
			if (this.ReadyState == Transport.ReadyStateEnum.OPEN)
				...
				logger.Info(string.Format("ignoring poll - transport state {0}", ReadyState));
		}
		...
		protected override void DoClose()
		{
			...
			else
			{
				...
				this.Once(Transport.EVENT_OPEN, closeListener);
			}
		}

		protected override void Write(ImmutableList<Packet> packets)
		{
			LogManager.GetLogger(Global.CallerName("", 0, "")).Info("Write packets.Count = " + packets.Count);
			this.Writable = false;
			Polling.SendEncodeCallback sendEncodeCallback = new Polling.SendEncodeCallback(this);
			Quobject.EngineIoClientDotNet.Parser.Parser2.EncodePayload(packets.ToArray<Packet>(), sendEncodeCallback);
		}
		...
		public string Uri()
		{
			...
			if (this.TimestampRequests)
				dictionary.Add(this.TimestampParam, DateTime.Now.Ticks.ToString() + "-" + Transport.Timestamps++);
		...
		public class SendEncodeCallback : IEncodeCallback
		{
			...
			{
				this.polling.DoWrite((byte[])data, () =>
				{
