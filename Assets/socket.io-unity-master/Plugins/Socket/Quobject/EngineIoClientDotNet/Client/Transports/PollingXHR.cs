...
namespace Socket.Quobject.EngineIoClientDotNet.Client.Transports
{
	public class PollingXHR : Polling
	{
		...
		protected PollingXHR.XHRRequest Request(PollingXHR.XHRRequest.RequestOptions opts)
		{
			...
			xhrRequest.On(Transport.EVENT_REQUEST_HEADERS, new PollingXHR.EventRequestHeadersListener(this)).On(Transport.EVENT_RESPONSE_HEADERS, new PollingXHR.EventResponseHeadersListener(this));
			return xhrRequest;
		}
		...
		protected override void DoWrite(byte[] data, ActionTrigger action)
		{
			...
			LogManager.GetLogger(Global.CallerName("", 0, "")).Info("DoWrite data = " + data);
			this.sendXhr = this.Request(opts);
			this.sendXhr.On(Transport.EVENT_SUCCESS, new PollingXHR.SendEventSuccessListener(action));
			this.sendXhr.On(Transport.EVENT_ERROR, new PollingXHR.SendEventErrorListener(this));
			this.sendXhr.Create();
		}
		...
		protected override void DoPoll()
		{
			...
			this.sendXhr.On(Transport.EVENT_DATA, new PollingXHR.DoPollEventDataListener(this));
			this.sendXhr.On(Transport.EVENT_ERROR, new PollingXHR.DoPollEventErrorListener(this));
			this.sendXhr.Create();
		}
		...
		private class SendEventErrorListener : IListener, IComparable<IListener>
		{
			...
			public void Call(params object[] args)
			{
				this.pollingXHR.OnError("xhr post error", args.Length == 0 || !(args[0] is Exception) ? null : (Exception)args[0]);
			}
		...
		private class DoPollEventErrorListener : IListener, IComparable<IListener>
		{
			...
			public void Call(params object[] args)
			{
				this.pollingXHR.OnError("xhr poll error", args.Length == 0 || !(args[0] is Exception) ? null : (Exception)args[0]);
			}
		...
		public class XHRRequest : Emitter
		{
			...
			public void Create()
			{
				...
				try
				{
					if (this.Data != null)
					{
						this.Xhr.ContentLength = Data.Length;
			...
			private void OnData(string data)
			{
				...
				this.Emit(Transport.EVENT_DATA, data);
				this.OnSuccess();
			}
			...
			{
				this.Emit(Transport.EVENT_ERROR, err);
			}

			private void OnRequestHeaders(Dictionary<string, string> headers)
			{
				this.Emit(Transport.EVENT_REQUEST_HEADERS, headers);
			}

			private void OnResponseHeaders(Dictionary<string, string> headers)
			{
				this.Emit(Transport.EVENT_RESPONSE_HEADERS, headers);
			}
