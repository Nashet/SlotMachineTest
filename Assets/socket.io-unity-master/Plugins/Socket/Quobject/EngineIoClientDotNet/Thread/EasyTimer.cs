...
namespace Socket.Quobject.EngineIoClientDotNet.Thread
{
	public class EasyTimer
	{
		...
		public EasyTimer(ActionTrigger method, int delayInMilliseconds, bool once = true)
		{
			...
			if (once)
				ThreadPool.QueueUserWorkItem(arg =>
				{
					...
					if (easyTimer.m_stop)
						return;
					EasyTimer.DoWork(method);
				});
			else
				this.m_timer = new Timer(new TimerCallback(EasyTimer.DoWork), method, 0, delayInMilliseconds);
		}
		...
		{
			ThreadPool.QueueUserWorkItem(new WaitCallback(EasyTimer.DoWork), action);
		}
	}
}