	...
	where T : IEquatable<T>
	{
		...
		public T[] ToArrayData(int startIndex, int length)
		{
			...
			for (int index = segmentIndex; index < this.m_Segments.Count; ++index)
			{
				...
				Array.Copy(segment.Array, segment.Offset + num, objArray, destinationIndex, length1);
		...
		public int SearchLastSegment(SearchMarkState<T> state)
		{
			...
			if (segment == null)
				return -1;
			int? nullable = Extensions.SearchMark<T>(segment.Array, segment.Offset, segment.Count, state.Mark);
		...
		public int CopyTo(T[] to, int srcIndex, int toIndex, int length)
		{
			...
			Array.Copy(arraySegmentEx.Array, sourceIndex, to, num1 + toIndex, length1);
			...
			for (int index = segmentIndex + 1; index < this.m_Segments.Count; ++index)
			{
				...
				Array.Copy(segment.Array, segment.Offset, to, num2 + toIndex, length2);
				num2 += length2;
				if (num2 >= length)
					break;
			}
