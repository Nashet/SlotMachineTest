	...
	public class WebSocket : IDisposable
	{
		...
		private EndPoint ResolveUri(string uri, int defaultPort, out int port)
		{
			...
			return !IPAddress.TryParse(this.TargetUri.Host, out address)
			  ? new DnsEndPoint(this.TargetUri.Host, port)
		...
		private TcpClientSession CreateClient(string uri)
		{
			...
			this.HandshakeHost = port != 80 ? this.TargetUri.Host + ":" + port : this.TargetUri.Host;
			return new AsyncTcpSession(this.m_HttpConnectProxy ?? endPoint);
		}
		...
		private TcpClientSession CreateSecureClient(string uri)
		{
			...
			this.HandshakeHost = port != 443 ? this.TargetUri.Host + ":" + port : this.TargetUri.Host;
			return new SslStreamTcpSession(this.m_HttpConnectProxy ?? endPoint);
		}
		...
		{
			this.CommandReader =
			   this.ProtocolProcessor.CreateHandshakeReader(this);
		...
		protected internal virtual void OnHandshaked()
		{
			...
			if (this.EnableAutoSendPing && this.ProtocolProcessor.SupportPingPong)
			{
				...
				this.m_WebSocketTimer = new Timer(new TimerCallback(this.OnPingTimerCallback),
				   ProtocolProcessor, this.AutoSendPingInterval * 1000, this.AutoSendPingInterval * 1000);
			}

			if (this.m_Opened == null)
				return;
			this.m_Opened(this, EventArgs.Empty);
		}
		...
		{
			this.Close(ProtocolProcessor.CloseStatusCode.NormalClosure, reason);
		}
		...
		public void Close(int statusCode, string reason)
		{
			...
			else
			{
				...
				this.m_WebSocketTimer =
				  new Timer(new TimerCallback(this.CheckCloseHandshake), null, 5000, -1);
		...
		private void FireClosed()
		{
			...
			if (closed == null)
				return;
			closed(this, (EventArgs)this.m_ClosedArgs ?? EventArgs.Empty);
		}
