...
namespace Socket.WebSocket4Net.Default
{
	public class JsonWebSocket : IDisposable
	{
		...
		private Dictionary<string, IJsonExecutor> m_ExecutorDict =
		  new Dictionary<string, IJsonExecutor>(StringComparer.OrdinalIgnoreCase);
		...
		public JsonWebSocket(
		  ...
		  : this(uri, subProtocol, cookies, null, string.Empty, string.Empty,
			version)
		{ }
		...
		private void m_WebSocket_MessageReceived(object sender, MessageReceivedEventArgs e)
		{
			...
			try
			{
				obj = executor.Type.IsSimpleType()
				  ? (json.GetType() != (object)executor.Type
					? Convert.ChangeType(json, executor.Type, null)
					: json)
				  : this.DeserializeObject(json, executor.Type);
			}
			catch (Exception ex)
			{
				this.m_WebSocket_Error(this, new ErrorEventArgs(new Exception("DeserializeObject exception", ex)));
				return;
			}
			...
			{
				this.m_WebSocket_Error(this, new ErrorEventArgs(new Exception("Message handling exception", ex)));
			}
		}
		...
		public void Send(string name, object content)
		{
			...
			if (content != null)
			{
				if (!content.GetType().IsSimpleType())
					this.m_WebSocket.Send(string.Format("{0} {1}", name, this.SerializeObject(content)));
				else
					this.m_WebSocket.Send(string.Format("{0} {1}", name, content));
			}
			else
		...
		private string Query<T>(string name, object content, IJsonExecutor executor)
		{
			...
			{
				if (!content.GetType().IsSimpleType())
					this.m_WebSocket.Send(string.Format("{0}-{1} {2}", name, num,
					   this.SerializeObject(content)));
				else
					this.m_WebSocket.Send(string.Format("{0}-{1} {2}", name, num, content));
			}
			else
				this.m_WebSocket.Send(string.Format("{0}-{1}", name, num));
		...
		private void RegisterExecutor<T>(string name, string token, IJsonExecutor executor)
		{
			lock (this.m_ExecutorDict)
			{
				if (string.IsNullOrEmpty(token))
					...
					this.m_ExecutorDict.Add(string.Format("{0}-{1}", name, token), executor);
			}
		}
