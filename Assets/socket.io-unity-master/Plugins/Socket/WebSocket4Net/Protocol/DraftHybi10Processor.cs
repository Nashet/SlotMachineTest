...
namespace Socket.WebSocket4Net.Protocol
{
	internal class DraftHybi10Processor : ProtocolProcessorBase
	{
		...
		public override void SendHandshake(WebSocket websocket)
		{
			...
			websocket.Items[this.m_ExpectedAcceptKey] = challenge;
			StringBuilder builder = new StringBuilder();
			if (websocket.HttpConnectProxy == null)
				builder.AppendFormatWithCrCf("GET {0} HTTP/1.1", websocket.TargetUri.PathAndQuery);
			else
				builder.AppendFormatWithCrCf("GET {0} HTTP/1.1", websocket.TargetUri.ToString());
			...
			builder.Append(string.Format("{0}: ", m_OriginHeaderName));
			...
			if (websocket.CustomHeaderItems != null)
			{
				for (int index = 0; index < websocket.CustomHeaderItems.Count; ++index)
				{
					...
					builder.AppendFormatWithCrCf("{0}: {1}", customHeaderItem.Key, customHeaderItem.Value);
				}
			}
		  ...
		  int length)
		{
			...
			numArray[1] = (byte)(numArray[1] | 128U);
		...
		public override void SendData(WebSocket websocket, IList<ArraySegment<byte>> segments)
		{
			...
			websocket.Client.Send(arraySegmentList);
		}
		...
		private void GenerateMask(byte[] mask, int offset)
		{
			...
			for (int index = offset; index < num; ++index)
				mask[index] = (byte)DraftHybi10Processor.m_Random.Next(0, byte.MaxValue);
		}
		  ...
		  int maskOffset)
		{
			for (int index1 = 0; index1 < length; ++index1)
			{
				...
				outputData[outputOffset++] = (byte)(rawData[index2] ^ (uint)mask[maskOffset + index1 % 4]);
			}
		}
	}
}