	  ...
	  byte.MaxValue,
	   0
		  ...
		  out string description)
		{
			...
			{
				if (data[index] != this.m_ExpectedChallenge[index])
				{
		...
		private byte[] GetResponseSecurityKey(string secKey1, string secKey2, byte[] secKey3)
		{
			...
			Array.Copy(bytes1, 0, source, 0, bytes1.Length);
			Array.Copy(bytes2, 0, source, bytes1.Length, bytes2.Length);
			Array.Copy(numArray, 0, source, bytes1.Length + bytes2.Length, numArray.Length);
