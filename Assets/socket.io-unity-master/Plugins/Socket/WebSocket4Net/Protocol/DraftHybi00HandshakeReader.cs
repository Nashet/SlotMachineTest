	...
	internal class DraftHybi00HandshakeReader : HandshakeReader
	{
		...
		private void SetDataReader()
		{
			this.NextCommandReader =
			   new DraftHybi00DataReader(this);
		}
		  ...
		  out int left)
		{
			if (this.m_ReceivedChallengeLength < 0)
			{
				...
				Buffer.BlockCopy(readBuffer, srcOffset, m_Challenges, 0, this.m_ExpectedChallengeLength);
			...
			{
				Buffer.BlockCopy(readBuffer, offset, m_Challenges, this.m_ReceivedChallengeLength,
				  length);
				left = 0;
			...
			Buffer.BlockCopy(readBuffer, offset, m_Challenges, this.m_ReceivedChallengeLength, count);
			left = length - count;
			this.SetDataReader();
			this.m_HandshakeCommand.Data = this.m_Challenges;
			return this.m_HandshakeCommand;
